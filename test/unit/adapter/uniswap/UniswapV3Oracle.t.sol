...
contract UniswapV3OracleTest is Test {
    ...
    function test_GetQuote_GetQuotes_RevertsWhen_InAmountGtUint128(FuzzableConfig memory c, uint256 inAmount) public {
        _deploy(c);
        inAmount = bound(inAmount, uint256(type(uint128).max) + 1, type(uint256).max);
        console2.log(oracle.base(), oracle.quote());
        console2.log(c.base, c.quote);
    ...
    function test_GetQuote_GetQuotes_RevertsWhen_NotSupported_Base(
    ...
    function test_GetQuote_GetQuotes_RevertsWhen_NotSupported_Quote(
        FuzzableConfig memory c,
        uint256 inAmount,
        address quote
    ) public {
