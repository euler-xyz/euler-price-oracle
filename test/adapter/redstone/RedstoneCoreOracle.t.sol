...
contract RedstoneCoreOracleTest is RedstoneCoreOracleHelper {
    ...
    function test_GetQuote_Integrity(FuzzableConfig memory c, FuzzableAnswer memory d, uint256 inAmount) public {
        ...
        assertEq(outAmount, (inAmount * d.price * 10 ** c.quoteDecimals) / 10 ** (8 + c.baseDecimals));
        assertEq(outAmountInv, (inAmount * 10 ** (8 + c.baseDecimals)) / (d.price * 10 ** c.quoteDecimals));
    }
    ...
    function test_GetQuotes_Integrity(FuzzableConfig memory c, FuzzableAnswer memory d, uint256 inAmount) public {
        ...
        uint256 expectedOutAmount = (inAmount * d.price * 10 ** c.quoteDecimals) / 10 ** (8 + c.baseDecimals);
        assertEq(bidOutAmount, expectedOutAmount);
        assertEq(askOutAmount, expectedOutAmount);

        (uint256 bidOutAmountInverse, uint256 askOutAmountInverse) = oracle.getQuotes(inAmount, c.quote, c.base);
        uint256 expectedOutAmountInverse = (inAmount * 10 ** (8 + c.baseDecimals)) / (d.price * 10 ** c.quoteDecimals);
