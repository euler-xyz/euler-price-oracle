...
/// @notice Oracle resolver for Euler lending products with an integrated sequencer uptime check.
...
contract EulerRouterWithSequencerCheck is Governable, IPriceOracle {
    /// @inheritdoc IPriceOracle
    string public constant name = "EulerRouterWithSequencerCheck";
    /// @notice The time period after the sequencer is back up during which txs will be rejected.
    uint256 private constant SEQUENCER_GRACE_PERIOD = 3600;
    /// @notice The Chainlink feed for checking L2 sequencer liveness.
    ...
    constructor(address _evc, address _governor, address _sequencerUptimeFeed) Governable(_evc, _governor) {
    ...
    /// @notice Perform a sequencer liveness check.
    /// @dev Reverts if the sequencer is down according to the Chainlink uptime feed.
