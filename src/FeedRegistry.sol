...
/// @notice Stores a list of feed identifiers used for deploying external oracles.
contract FeedRegistry is Governable {
    /// @notice FeedIdentifier configured for base/quote.
    /// @dev Note that FeedIdentifier can store both address and bytes32.
    mapping(address base => mapping(address quote => FeedIdentifier)) public getFeed;

    /// @notice Set a corresponding feed to base/quote.
    /// @param base The address of the base token.
    /// @param quote The address of the quote token.
    /// @param feed The feed identifier that corresponds to base/quote.
    ...
    function govSetFeeds(address[] calldata bases, address[] calldata quotes, FeedIdentifier[] calldata feeds)
        external
        onlyGovernor
    {
