...
/// Modified from `ChainlinkInfrequentOracle` to use nanoseconds for the staleness calculation.
contract ChainlinkInfrequentNanosecondOracle is BaseAdapter {
    /// @inheritdoc IPriceOracle
    string public constant name = "ChainlinkInfrequentNanosecondOracle";
    ...
    /// @dev Uses nanoseconds for the staleness calculation.
        ...
        returns (uint256)
    {
        ...
        (, int256 answer,, uint256 updatedAtNanos,) = AggregatorV3Interface(feed).latestRoundData();
        if (answer <= 0) revert Errors.PriceOracle_InvalidAnswer();
        // This feed reports `updatedAt` in nanoseconds; convert to seconds for the staleness check.
        uint256 updatedAt = updatedAtNanos / 1e9;
