...
abstract contract BasePythOracle is BaseOracle {
    struct Config {
        bytes32 feedId;
        uint8 decimals;
    }
    ...
    function govUnsetConfig(address token) external onlyGovernor {
        delete configs[token];
    }

    function getQuote(uint256, address, address) external view virtual returns (uint256);
    function getQuotes(uint256, address, address) external view virtual returns (uint256, uint256);
